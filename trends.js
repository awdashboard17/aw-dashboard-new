...
function getlastbuildid(jobname)
{
	var jenkinsapi = require('jenkins-api');
	var jenkins = jenkinsapi.init(jenkinurl);
	...
	jenkinsdata.on('update', function () {	
		...
		//console.log(jenkinsdata.data.id);
		gettrenddata(jobname, jenkinsdata.data.id);
	});	
	console.log("Out of everything");
}

function gettrenddata(jobname, maxjobcount)
{	
...
function getbuilddata(jobname, buildid)
{
